...
async def ask_phone(message: types.Message, state: FSMContext):
    ...
    # Запускаем дожим
    asyncio.create_task(send_payment_reminders(message.chat.id, state))

# 🔄 Функция для дожима (напоминания об оплате)
async def send_payment_reminders(user_id, state: FSMContext):
    for i in range(5):  # 5 напоминаний каждые 2 минуты
        await asyncio.sleep(30)  # Ждать 2 минуты
        current_state = await state.get_state()
        if current_state != OrderState.waiting_for_phone:
            break  # Если клиент уже оплатил, прекращаем напоминания

        objection_text = OBJECTION_MESSAGES[i % len(OBJECTION_MESSAGES)]  # Берем одно из возражений
